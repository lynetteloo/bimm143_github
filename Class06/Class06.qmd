...
Functions are how we get work done in R. We call functions to do everythign from reading data to doing analysis and outputting plots and results.
...
We will start simple with a simple version of the problem and the following example student vectors:
...
I have found the NA (TRUE) values from `is.na()`, now I want to make them equal to 0 (overwrite themmask them,etc.).
...
Now we can use the `apply()` to examine the correlation of every assignment in the masked gradebook to the overall score for each student in the class.
